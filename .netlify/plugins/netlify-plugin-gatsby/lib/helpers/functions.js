...
const writeFunction = async ({ renderMode, handlerName, appDir, functionsSrc, }) => {
    ...
    await (0, fs_extra_1.copyFile)((0, pathe_1.join)(__dirname, '..', '..', 'lib', 'templates', 'utils.js'), (0, pathe_1.join)(functionsSrc, handlerName, 'utils.js'));
};
...
const writeFunctions = async ({ constants, netlifyConfig, }) => {
...
const setupImageCdn = async ({ constants, netlifyConfig, }) => {
    ...
    await (0, fs_extra_1.copyFile)((0, pathe_1.join)(__dirname, '..', '..', 'src', 'templates', 'ipx.ts'), (0, pathe_1.join)(constants.INTERNAL_FUNCTIONS_SRC, '_ipx.ts'));
