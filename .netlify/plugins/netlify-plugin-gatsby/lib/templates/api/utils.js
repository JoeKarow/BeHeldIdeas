...
const createRequestObject = ({ event, context, }) => {
    const { path = '', multiValueQueryStringParameters, queryStringParameters, httpMethod, multiValueHeaders = {}, body, isBase64Encoded, } = event;
...
const createResponseObject = ({ onResEnd }) => {
    ...
    res.write = (chunk) => {
        ...
        response.body = buffer_1.Buffer.concat([
            buffer_1.Buffer.isBuffer(response.body)
                ? response.body
                : buffer_1.Buffer.from(response.body),
